	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (p.hasPermission("mokcore.user")) {
			...
			for (Player player : Bukkit.getOnlinePlayers()) {
				...
				if (player.getLocation().distance(p.getLocation()) <= 2) {
					...
					player.sendMessage(ChatColor.GRAY + "[" + ChatColor.YELLOW + "W" + ChatColor.GRAY + "] " + prefix + " " + main.getCharacterManager().getCharacter(p.getName()).getTown().getColor() + main.getCharacterManager().getCharacter(p.getName()).getName() + ChatColor.RESET + ": " + ChatColor.GRAY +  msg);
				}
			}
			return true;
		}
